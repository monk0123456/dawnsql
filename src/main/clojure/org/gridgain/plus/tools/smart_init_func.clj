...
(defn add_user_group [ignite group_id group_name user_token group_type schema_name]
    (cond (my-lexical/is-str-empty? schema_name) (throw (Exception. "schema_name 不存在！"))
          (my-lexical/is-eq? schema_name "public") (throw (Exception. "不能在 public schema_name 中添加用户组！"))
          (has_user_token_type group_type) (if (true? (has-schema-name ignite schema_name))
